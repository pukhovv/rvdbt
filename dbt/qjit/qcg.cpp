...
void QuickCG::Generate(qir::Region *r)
{
	QuickCG cg;

	u32 n_labels = 0;
	for (auto &bb : r->blist) {
		n_labels = std::max(n_labels, bb.GetId());
	}
	for (u32 i = 0; i < n_labels; ++i) {
		cg.labels.push_back(j.newLabel());
	}
}
...
void QuickCG::Emit_hcall(qir::InstHcall *ins)
{
	// args preallocated
	j.call(ins->stub);
}

void QuickCG::Emit_br(qir::InstBr *ins)
{
...
void QuickCG::Emit_brcc(qir::InstBrcc *ins)
{
...
void QuickCG::Emit_gbr(qir::InstGBr *ins)
{
...
void QuickCG::Emit_gbrind(qir::InstGBrind *ins)
{
...
void QuickCG::Emit_vmload(qir::InstVMLoad *ins)
{
	auto &vrd = ins->o[0];
	auto &vbase = ins->i[0];
	auto sgn = ins->sgn;
...
void QuickCG::Emit_vmstore(qir::InstVMStore *ins)
{
	auto &vbase = ins->i[0];
	auto &vdata = ins->i[1];
...
void QuickCG::Emit_setcc(qir::InstSetcc *ins)
{
...
void QuickCG::Emit_mov(qir::InstUnop *ins)
{
	j.emit(asmjit::x86::Inst::kIdMov, make_gpr(ins->o[0]), make_operand(ins->i[0]));
}

template <asmjit::x86::Inst::Id Op>
ALWAYS_INLINE void QuickCG::EmitInstBinopCommutative(qir::InstBinop *ins)
{
	// constfolded
	auto &vrd = ins->o[0];
	auto vs1 = &ins->i[0];
...
template <asmjit::x86::Inst::Id Op>
ALWAYS_INLINE void QuickCG::EmitInstBinopNonCommutative(qir::InstBinop *ins)
{
	// constfolded
	auto &vrd = ins->o[0];
	auto vs1 = &ins->i[0];
